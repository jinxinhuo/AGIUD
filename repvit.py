...
class RepViTModule(nn.Module):
    def __init__(self, ic, c, use_se, use_hs, s):
        super(RepViTModule, self).__init__()
        # building first layer
        layers = []
        input_channel = ic
        if ic == 3:
            patch_embed = torch.nn.Sequential(Conv2d_BN(ic, c // 2, 3, 2, 1), torch.nn.GELU(),
                                              Conv2d_BN(c // 2, c, 3, 2, 1))
            layers = [patch_embed]
            input_channel = c
        # building inverted residual blocks
        output_channel = _make_divisible(c, 8)
        exp_size = _make_divisible(input_channel * 2, 8)
        layers.append(RepViTBlock(input_channel, exp_size, output_channel, 3, s, use_se, use_hs))
